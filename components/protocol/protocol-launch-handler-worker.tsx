...
// Hook-only worker for protocol-launch (mailto:/webcal:) handling.
// Split off ProtocolLaunchHandlerProvider so the provider — which
// wraps the entire authenticated tree — doesn't statically pull
// lib/protocol-handlers/{mailto,webcal,session}.ts (~528 LOC combined)
// into every authenticated route's cold-load bundle. The worker is
// dynamic-imported by the provider with `ssr: false`, so the handler
...
export function ProtocolLaunchHandlerWorker(): null {
