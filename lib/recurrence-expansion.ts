...
function expandEvent(
  ...
  // for...in over the overrides Record avoids the Object.entries tuples-array.
  // Runs per recurring event during calendar expansion; consequential when
  // a calendar has many recurring events with overrides.
  for (const recurrenceId in overrides) {
    const override = overrides[recurrenceId] as Partial<CalendarEvent> & { excluded?: boolean };
    if (override.excluded) continue;
