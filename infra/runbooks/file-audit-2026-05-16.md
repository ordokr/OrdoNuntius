...
|---|---|---|---|---|
| 1 | `proxy.ts` | `lib/admin/csp-frame-origins.ts::getEnabledPluginFrameOrigins` | ~~verify memoized~~ — **VERIFIED CLEAN**: has 5-second TTL in-memory cache (`cachedAt`, `cachedOrigins`). | ~~medium~~ resolved |
| 2 | `proxy.ts` | `lib/setup/state.ts::detectSetupState` | ~~verify pure read~~ — **VERIFIED CLEAN**: `configManager.isSetupComplete()` is O(1) in-memory; `isConfigReadOnly()` is cached after first env read; no disk I/O on hot path. | ~~medium~~ resolved |
...
| 4 | `lib/admin/bootstrap-payload.ts` | `hooks/use-config.ts` | ~~duplicate `"__ORDO_BOOTSTRAP__"` literal~~ — **RESOLVED**: extracted to `lib/bootstrap-constants.ts` (dep-free); both consumers import from there. Client bundle no longer needs to know configManager exists just to read the script tag's `id`. | ~~low~~ resolved |
| 5 | `lib/admin/bootstrap-payload.ts` | `lib/auth/session-secret.ts::hasSessionSecret` | ~~called 2×~~ — **RESOLVED**: pulled into a single `sessionSecretAvailable` local. `hasSessionSecret` does a synchronous `readFileSync` when `SESSION_SECRET_FILE` is set, so the doubling was a real (if small) disk hit per SSR render on container deployments. | ~~trivial~~ resolved |
...
| 8 | `app/[locale]/login/page.tsx` | Same file | ~~`router.push('/')` after success redundant with useEffect~~ — **RESOLVED**: handlers now only push in add-account mode (where the useEffect is gated by `!isAddAccountMode`); normal mode defers to the useEffect, which preserves the saved-redirect-after-login path. Kills the double-navigation. | ~~trivial~~ resolved |
...
| 10 | `stores/auth-store.ts` | Same file (tier-2) | ~~Three login methods share ~80 lines of post-connect boilerplate~~ — **PARTIALLY RESOLVED**: extracted `finalizeBearerLogin()` shared by `loginWithOAuth` and `loginWithServerSso` (the two near-identical tails; ~60 lines deduped). Password login's tail stays inline — it's structurally different (identities arrive via promise, conditional session cookie write). | ~~medium~~ partially resolved |
...
## Files (hot-path-first, then alphabetical)
 ...
 11. [x] `stores/email-store.ts` — **4 deferred items resolved**: (a) `threadEmailsCache` now bounded LRU (cap 64, ~6 MB ceiling — delete-then-set + drop-oldest); (b) `spamUndoCache` mutations migrated from in-place `get().X.set/delete` to immutable copies via `set(state => ({ spamUndoCache: new Map(state.spamUndoCache) }))`; cache also resets on `selectMailbox` / `selectKeyword` so stale undo entries don't restore an email into the wrong mailbox; (c) `markAsRead` switched to optimistic update — UI now flips in ~0ms (was awaiting JMAP RTT), with rollback on server failure that restores the specific email's keywords, selectedEmail, and per-mailbox counters from snapshot; (d) **slice-split stages 1-5**: extracted unified-mailbox (`unified.ts`, 4 fields + 4 actions), thread-expansion (`thread.ts`, 3 fields + 4 actions), spam (`spam.ts`, 1 field + 4 actions), search (`search.ts`, 4 fields + 5 actions), and push notifications (`push.ts`, 3 fields + 5 actions including the heavyweight `handleStateChange` / `refreshCurrentMailbox`) into dedicated slices under `stores/email-slices/`. Shared `getNextSelectedEmail` helper extracted to `_helpers.ts` (used by core + spam slices, kept out of email-store to dodge circular imports). All slices mixed into `EmailStore` via interface extension + spread in `create`. Shrinks email-store.ts 2405→1822 (-583 lines, -24%). Remaining slice candidates (batch ops / mailbox management / selection) deferred — diminishing returns from here, slices increasingly read core state via wider StateCreator unions.
