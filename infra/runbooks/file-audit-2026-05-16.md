...
## Files (hot-path-first, then alphabetical)
 ...
 11. [x] `stores/email-store.ts` — **4 deferred items resolved**: (a) `threadEmailsCache` now bounded LRU (cap 64, ~6 MB ceiling — delete-then-set + drop-oldest); (b) `spamUndoCache` mutations migrated from in-place `get().X.set/delete` to immutable copies via `set(state => ({ spamUndoCache: new Map(state.spamUndoCache) }))`; cache also resets on `selectMailbox` / `selectKeyword` so stale undo entries don't restore an email into the wrong mailbox; (c) `markAsRead` switched to optimistic update — UI now flips in ~0ms (was awaiting JMAP RTT), with rollback on server failure that restores the specific email's keywords, selectedEmail, and per-mailbox counters from snapshot; (d) **slice-split stages 1+2**: extracted unified-mailbox state + 4 actions into `stores/email-slices/unified.ts` and thread-expansion state (3 fields) + 4 actions into `stores/email-slices/thread.ts`. Both slices mixed into `EmailStore` via interface extension + spread in `create`. Shrinks email-store.ts 2405→2251 (-154 lines). Stage 3+ (spam/search slices) deferred — incremental, each slice landed separately so blast radius stays bounded.
